        ...
        allTechs.forEach(t => {
            ...
            if (!hasSubs) {
                ...
                if (targetExercises.length > 0) {
                    const statuses = targetExercises.map(ex => ex.status).filter(s => ['high', 'medium', 'minimal', 'low'].includes(s));
...
recalculateMitreStatuses(mockMitreObj, exercises);
const end = performance.now();

console.log(`Time taken to recalculate mitre statuses for 100,000 exercises and 600 techniques: ${(end - start).toFixed(2)} ms`);
