...
assert("Average of 25.0% results in 'medium'", getAggStatus(['medium', 'low', 'low', 'low']) === 'medium'); // (50+0+0+0)/4 = 12.5% -> low
assert("Average of 25.0% threshold check", getAggStatus(['medium', 'low']) === 'medium'); // (50+0)/2 = 25% -> medium
assert("Average of 24.9% results in 'low'", getAggStatus(['medium', 'low', 'low', 'low']) === 'low'); // 12.5% -> low
assert("Average of 75% results in 'high'", getAggStatus(['high', 'medium', 'medium', 'medium']) === 'high'); // (100+50+50+50)/4 = 62.5% -> medium
assert("Average of 75% threshold check", getAggStatus(['high', 'high', 'high', 'low']) === 'high'); // (100+100+100+0)/4 = 75% -> high
...
const exercisesForGRS = [
    ...
    { ttp: "T1005", status: "N/A", campaign: "Campaign A", date: "2026-06-01" },      // Excluded
    { ttp: "T1006", status: "NA", campaign: "Campaign A", date: "2026-06-01" }        // Excluded
];

const grsResult = calculateGRS(exercisesForGRS);
// Valid = 3 (high, medium, low). Total points = 1.0 + 0.5 + 0 = 1.5. Score = 1.5/3 * 100 = 50%.
// If NA was not excluded, total validated = 6. Points = 1.5. Score = 1.5/6 * 100 = 25%.
assert("GRS calculation filters out 'na' case-insensitively (expected 50%)", grsResult === 50);

const campaignTrendResult = calculateCampaignTrend(exercisesForGRS);
assert("Campaign trends calculation filters out 'na' and does not penalize denominator", campaignTrendResult["Campaign A"].total === 3);
