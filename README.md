...
**Control Drift is built differently.** It goes beyond simply logging regressions; it enables defenders to actively challenge their security stack, discover hidden gaps, and learn more about adversary simulation through practical application.
...
Unlike heavy, legacy platforms that require multi-container Docker stacks, databases, and message brokers just to boot up, **Control Drift can run entirely in your browser.** By utilizing IndexedDB for a local-first experience, you can clone the repository, run `npm run dev`, and start logging gaps immediately. Need an enterprise setup? Seamlessly attach a Supabase or REST API backend with zero code changes.

### 2. Unmatched UX/UI
Security analysts deserve tools that feel as good as they work. Control Drift escapes the era of exhausting enterprise data tables. It features a premium, dark-mode glassmorphic interface, dynamic micro-animations, and intuitive workflows that significantly lower the barrier to entry for analysts beginning their Purple Teaming journey.

### 3. AI-Augmented Workflows
Control Drift includes plug-and-play AI-powered features such as a context-aware chatbot, TTP Auto-mapping, simulation strategy generation, executive report generation, and detection rule generation. Simply configure an AI integration within the application settings or via an external proxy.
...
- **MITRE ATT&CK Heatmap & Battle Globe**: Fully interactive 2D matrix and 3D globe providing a visual representation of your security posture across the MITRE ATT&CK framework, dynamically updated based on simulation outcomes.
  <br/>
...
- **Integrated AI Copilot**: Plug-and-play AI integration to automate TTP mapping, generate detection rules, synthesize executive summaries, and map viable attack paths.
