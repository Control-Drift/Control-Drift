...
describe('useExerciseActions', () => {
    ...
    const renderHookWithArgs = () => {
        return renderHook(() => useExerciseActions({
            dbAdapter: mockDbAdapter,
            exercisesPage: 1,
            exercisesLimit: 10,
            fetchExercisesPage: mockFetchExercisesPage,
    ...
    it('completeExercise correctly saves the exercise and resolves gaps if outcome is high', async () => {
        ...
        // Verify exercise was saved via the local adapter
        expect(mockDbAdapter.saveData).toHaveBeenCalledWith('exercises', expect.arrayContaining([
    ...
    it('updateExerciseValidation resolves the gap and recalculates parent coverage', async () => {
        const { result } = renderHookWithArgs();
        ...
        await act(async () => {
            const resolved = await result.current.updateExerciseValidation(
                mockGap, 
                'Prevented & Alerted', 
