...
export function useExerciseActions({
     ...
     * completeExercise
     * 
     * Submits a newly completed Exercise (atomic TTP test) into the database.
     ...
     * @param {Array} [tags=[]] - Custom tags to associate with the exercise.
     */
    const completeExercise = useCallback(async (ttp, finding, remediation, outcomeStatus, simulationName, severityOverride = null, environment = ['Windows Workstation'], coverageRating = null, outcomeString = null, tags = [], securityControls = [], skipFetch = false, dateOverride = null) => {
        ...
        if (!parseResult.success) {
            const errMsg = "Schema validation failed for Exercise.";
        ...
        const applyLocalUpdate = (adapterType) => {
            setExercises(prev => {
                ...
                if (adapterType === 'local' && dbAdapter) dbAdapter.saveData('exercises', next);
        ...
        if (dbAdapter && typeof dbAdapter.createExercise === 'function' && dbAdapter.type !== 'local') {
            try {
                await dbAdapter.createExercise(validExercise);
                applyLocalUpdate('remote');
            } catch (err) {
                console.error("completeExercise REST error:", err);
                if (addToast) addToast("Failed to save exercise. Database write error.", "error");
                throw err;
            }
        } else {
    ...
    }, [dbAdapter, exercisesPage, exercisesLimit, fetchExercisesPage, loadMitreCoverage, setAllExercisesData, setExercises, setGaps]);
     ...
     * 1. Appends the re-test notes to the original Exercise.
     ...
     */
    const updateExerciseValidation = useCallback(async (gapObj, newOutcomeStatus, validationNotes, validationDate) => {
        ...
        if (dbAdapter && typeof dbAdapter.fetchData === 'function' && dbAdapter.type === 'local') {
            ...
            // Fix state sync leak: update allExercisesData map properly
            setAllExercisesData(prevMap => {
                const nextMap = { ...prevMap };
            ...
            await fetchExercisesPage(exercisesPage, dbAdapter);
        } else {
            let modifiedExercises = [];
            
            // We must search the entire global map, not just the paginated 'exercises' list
            ...
            if (dbAdapter && typeof dbAdapter.updateExercise === 'function') {
                for (const ex of modifiedExercises) {
                    if (ex.id) {
                        try {
                            await dbAdapter.updateExercise(ex.id, {
                                status: ex.status,
                            ...
                            console.warn("Exercise update failed, likely due to unsupported column.", err);
                        }
                    }
                }
                await fetchExercisesPage(exercisesPage, dbAdapter);
    ...
    }, [dbAdapter, exercises, allExercisesData, gaps, exercisesPage, exercisesLimit, fetchExercisesPage, loadMitreCoverage, simulationSummaries, setSimulationSummaries, simulationEvidence, setExercises, setAllExercisesData, setGaps]);

    const revertExerciseValidation = useCallback(async (gapObj) => {
        ...
        if (dbAdapter && typeof dbAdapter.fetchData === 'function' && dbAdapter.type === 'local') {
            ...
            const updatedExercises = exercises.map(ex => {
                if (targetTTPs.includes(ex.ttp) && ex.simulation === simulationName) {
                    ...
                    if (dbAdapter && typeof dbAdapter.updateExercise === 'function') {
                        dbAdapter.updateExercise(updated.id, updated).catch(err => console.error("Error updating exercise:", err));
                    }
                    return updated;
                }
                return ex;
    ...
    }, [simulationSummaries, setSimulationSummaries, dbAdapter, exercises, allExercisesData, setExercises, setAllExercisesData]);
    ...
    const deleteSimulation = useCallback(async (simulationName) => {
        ...
        // 2. Delete from exercises (local state & DB)
        if (dbAdapter && typeof dbAdapter.fetchData === 'function' && dbAdapter.type === 'local') {
            const allExercises = await dbAdapter.fetchData('exercises') || [];
            const remainingExercises = allExercises.filter(ex => ex.simulation !== simulationName);
            await dbAdapter.saveData('exercises', remainingExercises);
            ...
            setExercises(prev => {
                const remaining = prev.filter(ex => ex.simulation !== simulationName);
                return remaining;
            });
            
            // Rebuild allExercisesData by filtering out the ones with simulationName
            setAllExercisesData(prevMap => {
                const nextMap = { ...prevMap };
                Object.keys(nextMap).forEach(key => {
                    if (nextMap[key].simulation === simulationName) {
                        delete nextMap[key];
                    }
                });
                return nextMap;
        ...
        if (gaps && setGaps) {
            if (dbAdapter && typeof dbAdapter.fetchData === 'function' && dbAdapter.type === 'local') {
                ...
                const remainingGaps = allGaps.filter(g => g.simulation !== simulationName);
                await dbAdapter.saveData('gaps', remainingGaps);
                setGaps(remainingGaps);
            } else {
                setGaps(prev => prev.filter(g => g.simulation !== simulationName));
            }
        }
    }, [setSimulationSummaries, dbAdapter, setAllExercisesData, setExercises, gaps, setGaps]);

    return {
        completeExercise,
        updateExerciseValidation,
        revertExerciseValidation,
        deleteSimulation
    };
}
