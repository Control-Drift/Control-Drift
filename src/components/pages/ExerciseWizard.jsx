...
import Step1BasicDetails from './ExerciseWizard/Step1BasicDetails';
import Step2Design from './ExerciseWizard/Step2Design';
import Step3Execute from './ExerciseWizard/Step3Execute';
import Step4Report from './ExerciseWizard/Step4Report';
 ...
 * 3. User sequentially executes and records outcomes ("Exercises") for each TTP.
 * 4. The wizard aggregates all atomic Exercises into a single cohesive Simulation
 *    via the `completeExercise` and `saveSimulationSummary` context actions.
 * 
 * NOMENCLATURE MAP:
 * - "Exercise": The individual TTP test recorded in the wizard steps.
...
3. Finally, at the very end of your response, you MUST output a single line starting with "MAPPED_IDS:" followed by a comma-separated list of the exact IDs you selected.`;
  ...
  const finishExercise = async () => {
     ...
     if (!reportData.executiveSummary || reportData.executiveSummary.trim() === '') {
         console.log('--- finishExercise: empty summary, aborting ---');
