...
def crop_favicon(filepath, threshold=50):
    try:
        ...
        cmin_sq = max(0, center_x - size // 2)
        rmin_sq = max(0, center_y - size // 2)
        cmax_sq = cmin_sq + size
        rmax_sq = rmin_sq + size
        
        cropped_img = img.crop((cmin_sq, rmin_sq, cmax_sq, rmax_sq))
        cropped_img.save(filepath)
        print(f"Successfully cropped {filepath} to {(cmin_sq, rmin_sq, cmax_sq, rmax_sq)}")
